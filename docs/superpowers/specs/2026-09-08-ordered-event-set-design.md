...
- Use portable C++23 and a simple `Makefile`, since CMake is not installed in the current environment.
- Compile with strict warnings.
- Run deterministic unit tests and an AddressSanitizer/UndefinedBehaviorSanitizer build where supported.
- The completed Track A extension adds deterministic build, lookup, read-heavy, and update-heavy benchmarks; raw CSV results; median/IQR analysis; five plots; an English report and PDF; and a video walkthrough outline.
